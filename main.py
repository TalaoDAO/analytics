...
def home(address):
            try:  
                ...
                con.row_factory = sql.Row
                cur = con.cursor()
                cur.execute("select * ,CASE WHEN applied =0 THEN 'pending' ELSE 'done' END AS status from (select a.relativeTo,a.hash,a.amount as amount,datetime(a.date) as date,b.applied,b.address,printf('%.3f', b.amount) as 'amountDiscount' ,b.hashPayement,c.discount,b.currency from transactions a, payements b, (select discount,id from usersWVouchers) c where a.hash=b.hash and c.id=a.relativeTo and b.forWho='player' and address='"+address+"')") 
                rows = cur.fetchall() 
                print("rows   --------2")
                sys.stdout.flush()
    #            print(rows[0])
